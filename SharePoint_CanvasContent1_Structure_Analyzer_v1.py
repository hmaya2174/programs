#!/usr/bin/env python3
"""
SharePoint CanvasContent1 Structure Analyzer v1
===============================================
...
1. Opens the normal SharePoint page first to warm/confirm the authenticated session.
2. Opens the generated REST URL in the normal Edge session.
...
------
One HTML file:
SharePoint_CanvasContent1_Structure_Report_YYYYMMDD_HHMMSS.html
...
APP_NAME = "SharePoint CanvasContent1 Structure Analyzer"
VERSION = "1.0"
MAX_SAMPLES = 5
...
def capture_sample(number: int, page_url: str) -> Sample:
    ...
    try:
        ...
        print(f"Sample {number}: warming authenticated SharePoint page...")
        open_in_edge(page_url)

        time.sleep(PAGE_WARMUP_SECONDS)

        print(f"Sample {number}: opening generated REST endpoint...")
        clear_clipboard()
        open_in_edge(sample.rest_url)

        time.sleep(REST_LOAD_SECONDS)
        ...
        if not raw:
            sample.error = "No response was captured from Edge."
            return sample
...
def main() -> int:
    ...
    print()
    print("IMPORTANT")
    print("1. Open Microsoft Edge normally.")
    print("2. Make sure you are already authenticated to the SharePoint site.")
    print("3. Leave Edge open.")
